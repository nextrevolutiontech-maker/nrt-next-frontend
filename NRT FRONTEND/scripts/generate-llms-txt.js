...
function generateLLMSTxt() {
  console.log('🤖 Generating /llms.txt and /llms-full.txt for GEO (Generative Engine Optimization)...');

  // 1. Generate llms.txt (Structured Outline)
  ...
  console.log(`✅ llms.txt generated at public/llms.txt`);

  // 2. Generate llms-full.txt (Full Text Dump of Knowledge Assets for RAG)
  ...
  mdxFiles.forEach(filePath => {
    ...
    if (data.evidence && Array.isArray(data.evidence)) {
      llmsFullTxt += `- **Citations & Evidence:**\n`;
      data.evidence.forEach(ev => llmsFullTxt += `  * ${ev.claim} (Source: ${ev.source || 'Internal Audit'})\n`);
    }

    llmsFullTxt += `\n### Article Content\n\n`;
    // Stripping heavy React component imports from MDX content for clean markdown digest
  ...
  console.log(`✅ llms-full.txt generated with ${compiledCount} knowledge assets at public/llms-full.txt`);
}
