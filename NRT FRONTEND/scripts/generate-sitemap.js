...
function generateSitemap() {
  ...
  // 2. Dynamic Routes from Knowledge Graph
  if (fs.existsSync(CACHE_FILE)) {
    try {
      ...
      console.log(`📌 Found ${nodes.length} dynamic content nodes from Knowledge Graph.`);
    } catch (err) {
      console.error('⚠️ Could not parse Knowledge Graph cache for sitemap:', err.message);
    }
  } else {
    console.warn('⚠️ knowledge-graph.json cache not found. Run build-graph.js first.');
  }
  ...
  console.log(`✅ sitemap.xml successfully generated with ${urls.length} total URLs at public/sitemap.xml`);
}
